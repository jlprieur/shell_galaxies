C************************************************************************
	SUBROUTINE POLYFIT2(XX,YY,NPOINT,KK,XC,SDEV,ERR)
C
C JLP
C Version 07-09-90
C	Fits a polynomial to real*8 arrays XX and YY
C	Degree : KK (less than 9)
C Same as POLYFIT1.FOR but with E04HEF
C************************************************************************
...
C***************************************************************************	
	...
	PARAMETER (IDIM=50)
	PARAMETER (LWW=8000,LV=20)
	REAL*8 XX(*),YY(*),WW(LWW),XC(20),SDEV(20)
	REAL*8 FJACC(IDIM,20),FVECC(IDIM),VV(LV,20)
	REAL*8 XX1(IDIM),YY1(IDIM)
	REAL*8 SUM,ETA,XTOL,STEPMX,SS(20),FSUMSQ
	REAL*4 ERR
	INTEGER*4 IW(100),LIW,MAXCALL
	CHARACTER ANS*1
	EXTERNAL LSQFUN,LSQHES,E04FDZ
...
C WARNING: 50 points maximum for E04HFF since I put LWW=8000!!!!!!!!!!!
C
	  IMOD=(NPOINT/50)+1
C Sum:
	  SUM=0.D0
...
C    Call NAG/E04HEF to find the minimum values
39	IFAIL=1
	ILSFUN2=0
	ILSHES2=0
	MAXCALL=50*N
	...
	STEPMX=100000.
	LJC=IDIM
	CALL E04HEF(M,N,LSQFUN,LSDHES,E04FDZ,-1,MAXCALL,
	1	ETA,XTOL,STEPMX,XC,FSUMSQ,FVECC,FJACC,LJC,
	1	SS,VV,LV,NITER,NF,IW,LIW,WW,LWW,IFAIL)
...
31	    FORMAT(' ITERATIONS :   ILSFUN2',I4,
	1	'  ILSHES2',I4)
	    PRINT 101,IFAIL
101	    FORMAT(' IN POLYNOMIAL FITTING, ERROR IN E04HEF,',
	1	'  IFAIL =',I3)
...
C****************************************************************************
	...
	PARAMETER (IDIM=50)
	INTEGER*4 LIW,LWW
	INTEGER*4 IFLAG,M,N,LJC,IW(LIW)
	REAL*8 FJACC(LJC,N),FVECC(M),XC(N),WW(LWW)
...
C*******************************************************
C LSQHES
C Subroutine called by E04HEF to calculate the elements of
...
C*******************************************************
	SUBROUTINE LSQHES(IFLAG,M,N,FVECC,XC,BB,LBB,
	1	IW,LIW,WW,LWW)
	IMPLICIT REAL*8 (A-H,P-Z)
	PARAMETER (IDIM=50)
	REAL*8 BB(LBB),FVECC(M),XC(N)
	REAL*8 XX(IDIM),YY(IDIM),WW(LWW)
	INTEGER*4 LIW,LWW
	INTEGER*4 IFLAG,M,N,LJC,IW(LIW)
	COMMON/MY_NAG/XX,YY,ILSFUN2,ILSHES2
...
C
	IMPLICIT REAL*8(A-H,P-Z)
	PARAMETER (IDIM=50,LIW=100,LWW=8000)
...
C Call LSQFUN to compute the Jaccobian matrix for the solution :
	LJC=IDIM
	CALL LSQFUN(IFLAG,M,N,XC,FVECC,FJACC,LJC,
	1	IW,LIW,WW,LWW)
	...
	IFAIL=0
C
C  Matrix inversion with F01ABF
C
