...
static int JLP_GETENV1(DEC_STRING(symbol), long *length, 
                       DEC_STRING(string), long *istat);
...
*****************************************************************/
 int JLP_GVM (float **pntr, long *memsize)
 {
   unsigned memsiz;
   memsiz = *memsize;
...
*****************************************************************/
 int JLP_GVMI (long **pntr, long *memsize)
 {
   unsigned memsiz;
   memsiz = *memsize;
   if((*pntr=(long *) malloc(memsiz)) != NULL)
	return(0);
...
* to rename a file 
************************************************************/
 int JLP_RENAME (DEC_STRING(infile),long *length1, 
                 DEC_STRING(outfile), long *length2, long *istat)
 {
...
*
****************************************************************/
 int JLP_SETENV (DEC_STRING(symbol), long *length1, DEC_STRING(string),
                long *length2, long *istat)
 {
 char command1[121], *pc;
 char mysymbol[41], mystring[81];
 int status;
 ...
 pc = mystring; while(*pc && *pc !=' ')pc++; *pc = '\0';

#ifdef VAX
 sprintf(command1,"define %s %s",mysymbol,mystring);
#else
 sprintf(command1,"setenv %s %s",mysymbol,mystring);
#endif
 command1[121]='\0';
/* JLP_SYSTEM returns 0 if success */
 *istat = JLP_SYSTEM(command1);
...
******************************************************************/
 int JLP_CTIME(DEC_STRING(string), long *istat)
 {
   long clock;
   char *p;
   *istat=1;
...
* New version
******************************************************************/
 int JLP_GETENV(DEC_STRING(symbol), long *length, DEC_STRING(string),
                long *istat)
 {
   long int iomode, fid, nochar, status;
   int i, i0, len;
   char *getenv(), *pc;
   char mysymbol[41], *p;
...
******************************************************************/
int JLP_DIR(DEC_STRING(input), long *length, DEC_STRING(output),
            long *nfiles, long *quiet, long *istat)
 {
 int iomode, fid, nochar, status;
 int i, eof_flag;
