...
C QUIET is set to true (no output of the names on the terminal)
	CALL JLP_DIRECTORY(GENERIC_NAME,NUMBER,.TRUE.)
...
C Input
	...
	ELSE
	  OPEN(1,FILE=NAME,READONLY,STATUS='OLD')
	ENDIF
...
C Output:
	...
	IF(II.NE.0)THEN
	  NAME=NAME(:II-1)//'                '
	ENDIF
	  OPEN(2,FILE=NAME,STATUS='NEW',CARRIAGECONTROL='LIST')
