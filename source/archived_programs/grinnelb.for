...
C format max 512 x 512
C
C Version of 24-11-87
C
C GRINNEL routines :
...
C
	...
	INTEGER*4 NPL,NL
	REAL*4 VAL(512),IMAGE(IDIM,IDIM)
	...
	COMMON/PARAM_GRI/IDW,IDH,IZCLAM,ICHA,ICH0,ICH1,ICH2,
     1	XMIN1,XMAX1,XMIN2,XMAX2,XMIN3,XMAX3,ILSTART
...
151	   FORMAT(' IMAGE :',A,/,' COMMENTS:',A,/,
     ...
     1	G12.5,2X,I4)
	  GO TO 80
C New Version of the titles: (in P. Bousquet's directory)
...
C ILSTART= Index of the first line displayed :
	ILSTART=1
C PB while reading with the cursor when IL.NE.1 ...
 	IF(NL.GT.512)THEN
	  PRINT *,' MORE THAN 512 LINES !'
	  PRINT *,' PLEASE ENTER THE FIRST LINE TO BE DISPLAYED :'
	  READ(5,*) ILSTART
	ENDIF
 
C Reading a sample of the image :
	NPL1=MIN0(NPL,512)
	NL1=MIN0(NL-ILSTART+1,512)
...
C Reading the input file and displaying the image line per line :
	DO 600 IY=ILSTART,NL1+ILSTART-1
...
C Plane 0:
	CALL GRWLW(IBUFF1,IXSTART,IYNOW,ILEN,IDW,IDH,ICH0,IZCLAM)
	CALL GRSBFD
...
C Plane 1:
	CALL GRWLW(IBUFF1,IXSTART,IYNOW,ILEN,IDW,IDH,ICH1,IZCLAM)
	CALL GRSBFD
...
C Plane 2:
	CALL GRWLW(IBUFF1,IXSTART,IYNOW,ILEN,IDW,IDH,ICH2,IZCLAM)
	CALL GRSBFD
...
C PB while reading with the cursor when IL.NE.1 ...
	...
	IF(IOP.EQ.8)THEN
C Calling Patch :
	   CALL PATCH2(IMAGE,NPL,NL,ILSTART,IDIM)
	ELSE
