C++************************************************************************
C Program to set up initial conditions for the cluster
C filament simulations.
C
C Isotropic cluster and isotropic filament galaxy
C
C From Peter Quinn Sept. 1987
C
C@@@ JLP JUNE 1989 : added dispersion in velocities at a given radius
C************************************************************************
...
C (Quinn' extent=10.)
	...
	REAL lum_mass_member,lum_mass_filgal
 
        CHARACTER*1 orbit_type
...
C	READ(5,5)ans4
   ...
   8         FORMAT(1x,'setup>>            member half-light radius: '$)
	...
	WRITE(6,9)
   9	FORMAT(1x,'setup>> Supply the filament galaxy luminous mass : '$)
	READ(5,*)lum_mass_filgal
	WRITE(6,10)
  10	FORMAT(1x,'setup>> Filament galaxy half-light radius  : '$)
	READ(5,*)re_filgal
	WRITE(6,11)
  11	FORMAT(1x,'setup>> Distance of the filament ',
     1	'galaxy from the centre : '$)
	READ(5,*)r_filgal
 
	IF(r_filgal.ge.outer_radius_cluster)THEN
	     contained_cluster_mass=total_mass_cluster
           ELSE
             ratio1=r_filgal/core_radius_cluster
	...
	PRINT *,' '
	WRITE(6,12)
  12	FORMAT(1x,'setup>> Filament galaxy orbit : c,p,e : ? : '$)
	READ(5,5)orbit_type
  ...
  20         FORMAT(1x,'setup>> approximate peri-distance : '$)
	...
	PRINT *,' '
	WRITE(6,14)
  14	FORMAT(1x,'setup>> Number of particles in filament galaxy : '$)
	READ(5,*)nbodies
 
	PRINT *,' '
	PRINT *,'setup>> Beginning Setup << '
 
C	CALL gen_filgal(nbodies,re_filgal,lum_mass_filgal,
C    &                  gamma_halo,extent,xp,yp,zp,vxp,vyp,vzp)
	CALL gen_plumgal(nbodies,re_filgal,lum_mass_filgal,
...
C Write iteration,time
	...
	WRITE(10,*)nbodies,lum_mass_filgal
	WRITE(10,*)re_filgal,gamma_halo,extent
        WRITE(10,*)r_filgal
	WRITE(10,*)orbit_type
...
C***************************************************************************
C
C	Subroutine to generate the filament galaxy as a simple
...
C***************************************************************************
	...
	SUBROUTINE gen_filgal(nbodies,re_filgal,lum_mass_filgal,
     &                        gamma_halo,extent,xp,yp,zp,vxp,vyp,vzp)
	...
	REAL lum_mass_filgal,lum_mass_filgal_scaled
 
	iseed=1234567
	pi=2.*asin(1.)
	rmax=rmax_factor*re_filgal
...
C and theta (projection uniform between -1. and 1.)
	...
	PRINT *,' '
	PRINT *,'gen_filgal>>',nbodies,' Particles set up <<'
	
C Recentrering the centre of mass to (0,0,0)
 	CALL  cmt_x(nbodies,xp,yp,zp,radius)
 
C Computing the central velocity dispersion :
	gamma_halo=scale*re_filgal
	lum_mass_filgal_scaled=lum_mass_filgal/unit_mass
	v_dispersion_filgal=sqrt(pi*G*lum_mass_filgal_scaled/
     &                          (32.*re_filgal))
	rho_0_halo=v_dispersion_filgal**2/(2.*pi*G*gamma_halo**2)
	...
	ratio_masses=halo_mass/lum_mass_filgal_scaled
	rscale=r_chi/gamma_halo
	v_dispersion_unscaled=v_dispersion_filgal*unit_vel
 
	PRINT *,' '
	PRINT *,'gen_filgal>---> filament galaxy velocity dispersion : ',
     &                        v_dispersion_unscaled
	PRINT *,'gen_filgal>---> extent of halo around filament galaxy : ',
     &                        r_chi
	PRINT *,'gen_filgal>---> total halo mass : ',halo_mass_unscaled
	PRINT *,' '
 
C Random generation of the velocities: (v_dispersion_filgal)
...
c	      ENDIF
	    phi_filgal=-G*lum_mass_filgal_scaled
     &                  /sqrt((radius(i)**2+re_filgal**2))
	
	    phi_total=phi_filgal
C	    phi_total=phi_halo+phi_filgal
	    v_total=sqrt(abs(phi_total))
 
C Gaussian NAG random generator :
C (sigma decreasing linearily with the distance)
	    sigma=v_dispersion_filgal
     1	*(gamma_halo-radius(i))/gamma_halo
	    sigma=max(0.,sigma)
C	    v_total=g05ddf(dble(v_total),dble(sigma))
	    call jlp_random_gauss(work)
            v_total=v_total+sigma*work
...
C***************************************************************************
	...
	SUBROUTINE gen_plumgal(nbodies,re_filgal,lum_mass_filgal,
     &                        gamma_halo,extent,xp,yp,zp,vxp,vyp,vzp)
...
C When stability=1. the particle velocity is set to the escape velocity
	PRINT *,' '
	PRINT *,'gen_filgal>---> stability factor  (10?) : '
	READ(5,*) stability_factor
...
C	 call g05cbf(0)
	...
	DO i=1,nbodies
     ...
     &                  /sqrt(radius(i)**2+re_filgal**2)
	    ...
	    v_total=sqrt(abs(phi_filgal))/stability_factor
